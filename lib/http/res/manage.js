...
var manage = function () {
	var manage = angular.module("danmu.manage", ["ui.bootstrap", "manageControllers"]);
	var manageControllers = angular.module('manageControllers', []);
	var registerInit = []; // 用于初始化回调

	// alternatively, register the interceptor via an anonymous factory

	manage.config(['$httpProvider', function ($httpProvider) {
		$httpProvider.interceptors.push(function ($q, $rootScope) {
			return {
				...
				'response': function response(_response) {
					$rootScope.err.code = _response.status;
					$rootScope.err.desc = "";
	...
	manageControllers.controller("MainCtrl", function ($scope, $http, $rootScope) {
		$scope.accordion = {
			closeOther: false,
			openInfo: true,
			openDanmu: false,
		...
		$scope.enterRoom = function (password) {
			...
			for (var object in registerInit) {
				registerInit[object].call();
			}$scope.isLogin = true;
	...
	manageControllers.controller("DanmuCtrl", function ($scope, $http) {
		$scope.danmu = {};
	...
	manageControllers.controller("BlockCtrl", function ($scope, $http) {
		$scope.block = {};
	...
	manageControllers.controller("ConfigCtrl", function ($scope, $http) {
		$scope.config = {};
	...
	manageControllers.controller("PermissionCtrl", function ($scope, $http) {
		$scope.config = {};
	...
	manageControllers.controller("PasswordCtrl", function ($scope, $http) {
